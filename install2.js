...
function log(msg) {
  fs.appendFileSync('install2.log', msg + '\n');
}

try {
  log('Installing backend...');
  execSync('npm install', { cwd: path.join(__dirname, 'backend'), shell: true });
  log('Backend installed successfully.');
  
  log('Installing frontend...');
  execSync('pnpm install', { cwd: path.join(__dirname, 'frontend'), shell: true });
